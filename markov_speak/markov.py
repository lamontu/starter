...
class Markov(object):
    ...
    def __init__(self, filepath=None, mode=0, coding='utf-8'):
        self.dictLen = 0
        self.Cap = []
        self.mode = mode  # English: mode=0; Chinese: mode=1
        self.coding = coding
        self.dic = {}
            ...
            for line in f.readlines():
                ...
                if mode == 0:
                    ...
                    for sentence in sentences:
                        effect_words = filter(
                            lambda x: x != '', sentence.split(' '))
                        sentences_words += list(effect_words)
                    if len(sentences_words) > 2:
                        for i in range(len(sentences_words) - 2):
                            keypair = sentences_words[i] + ' ' + sentences_words[i + 1]
                            if keypair[0].isupper():
                                self.Cap.append(keypair)
                            if self.dic.get(keypair) is None:
                                self.dic[keypair] = [sentences_words[i + 2]]
                            elif sentences_words[i + 2] not in self.dic[keypair]: 
                                    self.dic[keypair].append(sentences_words[i + 2])
                else:
                    ...
                    for sentence in sentences:
                        ...
                        if len(words) > 2:
                            ...
                            for i in range(len(words) - 2):
                                ...
                                elif words[i + 2] not in self.dic[keypair]: 
