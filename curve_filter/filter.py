"""
This program can add filter to the phoo.

Usage: python filter.py [options] <curves> <filepath>
...
class FilterManager(object):
    ...
    def apply_filter(self, filter_name, image_array):
        if image_array.ndim < 3:
            raise Exception('Photos must be in color,'
                             + ' meaning at least 3 channels')
        else: # Not sure about the function
...
def main():
    ...
    output_name = '%s_%s.png' % (get_name(args['<filepath>']),
                                 get_name(args['<curves>']))
    im.save(output_name)
